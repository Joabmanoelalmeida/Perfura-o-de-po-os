...
#c) A eficiência de remoção de sólidos assumindo fluxo ao redor da partícula turbulento (Cr=1,6);
...
m_anular = (Q_nova * m_fluido + Q_cascalho * 8.34*m_cascalho) / (Q_nova + Q_cascalho)
...
for m_fluido in m_fluido_values:
    # Recalcular a massa específica aparente do fluido no anular
    m_anular = (Q * m_fluido + Q_cascalho *8.34* m_cascalho) / (Q + Q_cascalho)
...
# Taxas de penetração
taxas_penetração = [10, 20, 30, 40]  # em m/h
ecd_fundo_values = []
ecd_sapata_values = []
